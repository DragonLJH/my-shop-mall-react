...
function TopNav() {
    ...
    const topNavContext = useContext(exportContext);
    const user = topNavContext.user || localStorage.getItem("user");
    const [form] = Form.useForm();
    const onFinish = (values) => {
        axios.get("http://localhost:8787/user/queryUserByUserName", { params: { userName: values.username, userPassword: values.password } })
            .then((myData) => {
                if (myData.data) {
