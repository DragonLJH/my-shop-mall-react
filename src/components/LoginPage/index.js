...
import { Form, Input, Button, Tabs, Row, Col, message, Divider } from 'antd';
...
function LoginPage() {
    ...
    //切换 清空 表单
    const callback = (key) => { LonReset(); RonReset(); }
    //登录 表单
    const [Lform] = Form.useForm();
    //注册 表单
    const [Rform] = Form.useForm();
    const myConsumer = useContext(exportContext);
    const layout = { labelCol: { span: 8, }, wrapperCol: { span: 8, }, };
    const tailLayout = { wrapperCol: { offset: 8, span: 16, }, };
    //登录 验证
    const LonFinish = (values) => {
        LonReset();
        console.log(values)
        axios.get("http://localhost:8787/user/queryUserByUserName", { params: { userName: values.userName, userPassword: values.userPassword } })
            .then((myData) => {
                console.log(myData)
                if (myData.data) {
                    myConsumer.Login(() => {
    ...
    //注册 验证
    const RonFinish = (values) => {
        RonReset();
        axios.post("http://localhost:8787/user/insertUser", qs.stringify({ userName: values.userName, userPassword: values.userPassword }))
            .then((myData) => {
                console.log(myData)
                if (myData.data) {
